...
- Список обрабатывается потоково; добавление — append в `frt.list`; удаление — однопроходное (без множественных `sed -i`).
- В dnsmasq увеличен `dns-forward-max` (1024). Домены попадают в ipset через dnsmasq при запросе.
